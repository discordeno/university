...
import { DiscordVerificationLevels,DiscordDefaultMessageNotificationLevels,DiscordExplicitContentFilterLevels,Collection,DiscordGuildFeatures,DiscordMfaLevels,DiscordSystemChannelFlags,Channel,DiscordPremiumTiers,WelcomeScreen,StageInstance,PresenceUpdate,Emoji,Guild,snowflakeToBigint,cache,DiscordImageSize,DiscordImageFormat,ModifyGuild,GetGuildAuditLog,CreateGuildBan,GuildMember,iconBigintToHash } from "../../deps.ts";
...
export class DiscordenoGuild extends Base {
    ...
    update(payload: Guild) {
      ...
      for (const role of payload.roles) {
        this.roles.set(snowflakeToBigint(role.id), new DiscordenoRole(this.client, role, this.id));
      }
  
      for (const voiceState of payload.voiceStates || []) {
        this.voiceStates.set(snowflakeToBigint(voiceState.userId), new DiscordenoVoiceState(this.client, this.id, voiceState));
      }
